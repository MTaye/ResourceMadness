...
import com.mtaye.ResourceMadness.Item;
...
public class TextHelper {
	...
	//Include Item
	public static String includeItem(Item rmItem){
	...
	public static String getStringSortedItems(List<ItemStack> items, int typeLimit){
		String strItems = "";
		HashMap<Integer, ItemStack> hashItems = InventoryHelper.combineItemsByItemStack(items);
		
		Integer[] array = hashItems.keySet().toArray(new Integer[hashItems.size()]);
		Arrays.sort(array);
		if(array.length>typeLimit){
			for(Integer id : array){
				strItems += ChatColor.WHITE+""+id+includeItem(new Item(hashItems.get(id)))+ChatColor.WHITE+", ";
			}
		}
		else{
			for(Integer id : array){
				strItems += ChatColor.WHITE+""+Material.getMaterial(id)+includeItem(new Item(hashItems.get(id)))+ChatColor.WHITE+", ";
			}
		}
	...
	public static String keepFirst(String string, String str){
		return keepFirst(string, str, str, "");
	}
	
	public static String keepFirst(String string, String str, String strReplace){
		return keepFirst(string, str, strReplace, "");
	}
	
	public static String keepFirst(String string, String str, String strReplace, String replace){
		String result = string;
		if(result.contains(str)){
			int i = result.indexOf(str);
			result = result.replace(str, replace);
			result = result.substring(0, i)+strReplace+result.substring(i, result.length());
		}
		return result;
	}
}
