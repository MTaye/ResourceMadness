...
public class PartList {
	...
	public PartList(Block b, RM plugin){
		init(b, null, plugin);
	}
	public PartList(Block b, Block bRemove, RM plugin){
	...
	*/
	...
	private List<Block> fetchParts(Block b, Material mat, Boolean trim, Block... faces){
		...
		if(mat==Material.WALL_SIGN){
				for(int i=0; i<facings.size(); i++){
					if((facings.get(i).getType()==Material.WALL_SIGN)&&(Dir.getDirByData(facings.get(i).getData()).getOpposite()==Dir.values()[i])){
	...
	public Block getMainBlock(Block b){
		switch(b.getType()){
			case WALL_SIGN:
				Dir dir = Dir.getDirByData(b.getData());
	...
	//Get Sign Block
	public static Block getSignBlockByDir(Block b, Dir dir){
		switch(dir){
			case NORTH:
	...
	public List<Team> fetchTeams(){
		...
		for(List<Block> blocks : blockList){
			...
			teams.add(new Team(DyeColor.getByData(blocks.get(Part.WOOL.ordinal()).getData()), (Chest)blocks.get(Part.CHEST.ordinal()).getState(), plugin));
		}
		return teams;
	}
	...
	public String getTextBlockList(boolean allowNull){
		...
		return TextHelper.stripLast(line, ",");
	}
	
	//Match Part List
	public Boolean matchPartList(PartList bl){
	...
	public static Boolean matchPartList(PartList bl1, PartList bl2){
		//plugin.getServer().broadcastMessage(plugin.getTextList(bl1.getChestList(),true)+":"+plugin.getTextList(bl2.getChestList(),true));
		//plugin.getServer().broadcastMessage(plugin.getTextList(bl1.getSignList(),true)+":"+plugin.getTextList(bl2.getSignList(),true));
		//plugin.getServer().broadcastMessage(plugin.getTextList(bl1.getWoolList(),true)+":"+plugin.getTextList(bl2.getWoolList(),true));
